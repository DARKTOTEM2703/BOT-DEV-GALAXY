...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option
        .setName("user")
        .setDescription("El usuario para dar un beso")
        .setRequired(true)
    ),
  async execute(interaction) {
    ...
    const embedKiss = new EmbedBuilder()
      ...
      .setDescription(
        `**${interaction.user.username}** ha besado a **${member.username}**`
      )
    ...
    const embedHug = new EmbedBuilder()
      .setColor("#ff0000") // Color del embed (puedes cambiarlo a tu preferencia)
      .setDescription(
        `**${interaction.user.username}** ha abrazado a **${member.username}**`
      )
    ...
    //await wait(2500);
    const preparandoMsg = await interaction.editReply(
      `:satellite: Preparando el comando, está dandole un beso...`
    );
